    ...
    preloadZoneData(null, function (err, data) {
        ...
        async.forEach(uuids, function (uuid, cb) {
            loadVM(uuid, data, function (error, obj) {
                ...
                for (ip in ips) {
                    if (ips[ip] !== 'dhcp' && matcher(obj, {'nics.*.ip': ips[ip]})) {
...
function createVolumes(payload, callback)
{
    ...
    if (p.hasOwnProperty('used_disk_indexes')) {
        used_disk_indexes = p.used_disk_indexes;
    }
...
function resizeDisks(disks, updates, callback)
{
    ...
    async.forEach(vols, resize, function (err) {
        if (err) {
...
function applyUpdates(oldobj, newobj, payload, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            async.forEach(keys, function(prop, c) {
                rctl = rctls[prop][0];
                if (rctls[prop][1]) {
                    factor = rctls[prop][1];
                } else {
